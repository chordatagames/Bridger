...
{
	public static float Angle(Vector2 direction, Vector2 relation, bool inner)
	{
		float angle = Vector2.Angle(direction,relation);
		return ((inner) ? angle : (relation.y >= 0) ? angle : 360-angle );
	}
